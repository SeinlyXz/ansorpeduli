<script lang="ts">
	...
	const handleImage = async () => {
		...
		if(images.length < 3){
			images = [...images, `data:image/jpeg;base64,${imageData}`];
			console.log(images)
		} else {
			return
		}
			...
			<section class="flex gap-x-3 justify-between">
				...
				{:else}
					{#each images as image, i}
						<div class="w-52 h-24 bg-gray-100 border rounded-xl relative">
							<button class="absolute -right-2 -top-2" type="button" aria-label="Remove image">
